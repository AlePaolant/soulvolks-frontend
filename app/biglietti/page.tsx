...
const labelClass = `
  ...
  text-[var(--nero)]/40 mb-1 font-poppins font-medium
`
  ...
  return (
    <PayPalScriptProvider options={{ clientId: PAYPAL_CLIENT_ID, currency: 'EUR' }}>
      <main className="min-h-screen bg-[var(--panna)]">
        ...
        {/* ── HERO ─────────────────────────────────────────── */}
        <section className="relative bg-[var(--scuro)] overflow-hidden">
          {/* Sfondo texture grana */}
          <div className="absolute inset-0 opacity-[0.04]"
            style={{ backgroundImage: 'url("data:image/svg+xml,%3Csvg viewBox=\'0 0 256 256\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cfilter id=\'noise\'%3E%3CfeTurbulence type=\'fractalNoise\' baseFrequency=\'0.9\' numOctaves=\'4\' stitchTiles=\'stitch\'/%3E%3C/filter%3E%3Crect width=\'100%25\' height=\'100%25\' filter=\'url(%23noise)\'/%3E%3C/svg%3E")', backgroundSize: '200px' }} />
          
          {/* Striscia rossa diagonale */}
          <div className="absolute bottom-0 left-0 right-0 h-[6px] bg-[var(--rosso)]" />
          
          <div className="relative z-10 max-w-7xl mx-auto px-8 py-20 md:py-28">
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-8">
              <div>
                <p className="font-poppins italic text-[var(--panna-chiaro)]/40 text-base mb-4 tracking-widest">
                  EST. 2023 — MOLISE
                </p>
                <h1 className="font-droid text-[var(--panna-chiaro)] leading-none"
                  style={{ 
                    fontSize: 'clamp(4rem, 14vw, 12rem)',
                    letterSpacing: '-0.03em',
                    textShadow: '6px 6px 0px rgba(225,39,19,0.3)'
                  }}>
                  MATESE<br />
              ...
              <div className="md:text-right md:pb-4 space-y-4 max-w-sm">
                <div className="flex items-center md:justify-end gap-3 text-[var(--panna-chiaro)]/60">
                  <Calendar size={16} />
                  <span className="font-poppins text-sm uppercase tracking-widest">7 · 8 · 9 Agosto 2026</span>
                </div>
                <div className="flex items-center md:justify-end gap-3 text-[var(--panna-chiaro)]/60">
                  <MapPin size={16} />
                  <span className="font-poppins text-sm uppercase tracking-widest">Campitello Matese (CB)</span>
                </div>
                <p className="font-droid italic text-[var(--panna-chiaro)]/80 text-lg leading-relaxed">
                  Auto d'epoca, musica dal vivo,<br />mercatini vintage e tanto altro.
                </p>
                <div className="inline-block border border-[var(--rosso)] px-6 py-3">
                  <span className="font-droid text-[var(--rosso)] text-2xl">€20</span>
                  <span className="font-poppins text-[var(--panna-chiaro)]/50 text-sm ml-2">/ 3 notti · camping incluso</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <form onSubmit={handleFormSubmit}>
              ...
              <div className="mb-16">
                <div className="flex items-center gap-4 mb-2">
                  ...
                  <span className="font-poppins text-[10px] uppercase tracking-[0.3em] text-[var(--nero)]/40">
                    Passo 1 di 2
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <button type="button" onClick={() => setFormData({ ...formData, tipo: 'volkswagen' })}
                    className={`relative p-8 border-2 text-left transition-all duration-300 group ${
                    ...
                    <div className={`absolute top-4 right-4 w-3 h-3 rounded-full border-2 ${
                      formData.tipo === 'volkswagen' ? 'bg-[var(--rosso)] border-[var(--rosso)]' : 'border-[var(--nero)]/30'
                    }`} />
                    <Car size={32} className={`mb-6 ${formData.tipo === 'volkswagen' ? 'text-[var(--rosso)]' : 'text-[var(--nero)]/40'}`} />
                    <h3 className="font-poppins font-bold text-xl uppercase tracking-wider mb-2">
                      Volkswagen
                    </h3>
                    <p className={`font-poppins text-sm font-light ${formData.tipo === 'volkswagen' ? 'text-[var(--panna)]/60' : 'text-[var(--nero)]/40'}`}>
                      Maggiolino, T1, T2, T3 e altri modelli d'epoca
                    </p>
                    <div className={`mt-6 inline-block px-4 py-1 text-xs uppercase tracking-widest font-poppins font-bold ${
                      formData.tipo === 'volkswagen' ? 'bg-[var(--rosso)] text-white' : 'bg-[var(--nero)]/10 text-[var(--nero)]/50'
                    }`}>
                  ...
                  <button type="button" onClick={() => setFormData({ ...formData, tipo: 'standard' })}
                    className={`relative p-8 border-2 text-left transition-all duration-300 group ${
                      formData.tipo === 'standard'
                        ? 'border-[var(--nero)] bg-[var(--nero)] text-[var(--panna)]'
                        : 'border-[var(--nero)]/20 hover:border-[var(--nero)]/60 bg-white'
                    }`}>
                    <div className={`absolute top-4 right-4 w-3 h-3 rounded-full border-2 ${
                      formData.tipo === 'standard' ? 'bg-[var(--rosso)] border-[var(--rosso)]' : 'border-[var(--nero)]/30'
                    }`} />
                    <Tent size={32} className={`mb-6 ${formData.tipo === 'standard' ? 'text-[var(--rosso)]' : 'text-[var(--nero)]/40'}`} />
                    <h3 className="font-poppins font-bold text-xl uppercase tracking-wider mb-2">
                      Auto / Camper / Tenda
                    </h3>
                    <p className={`font-poppins text-sm font-light ${formData.tipo === 'standard' ? 'text-[var(--panna)]/60' : 'text-[var(--nero)]/40'}`}>
                      Qualsiasi altro veicolo o campeggio con tenda
                    </p>
                    <div className={`mt-6 inline-block px-4 py-1 text-xs uppercase tracking-widest font-poppins font-bold ${
                      formData.tipo === 'standard' ? 'bg-[var(--rosso)] text-white' : 'bg-[var(--nero)]/10 text-[var(--nero)]/50'
                    }`}>
                      Zona B · Area Camping
                    </div>
                  </button>
              ...
              {formData.tipo === 'volkswagen' && (
                <div className="mb-12 p-8 bg-[var(--nero)] relative overflow-hidden">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                    <div>
                      <label className="block text-[10px] uppercase tracking-[0.2em] text-[var(--panna-chiaro)]/40 mb-1 font-poppins font-medium">Modello *</label>
                    ...
                    <div>
                      <label className="block text-[10px] uppercase tracking-[0.2em] text-[var(--panna-chiaro)]/40 mb-1 font-poppins font-medium">Anno *</label>
                      <input name="anno" type="number" min="1945" max="1995" value={formData.anno} onChange={handleChange} required
                        className="w-full bg-transparent border-b-2 border-[var(--panna-chiaro)]/20 px-0 py-3 text-[var(--panna-chiaro)] placeholder-[var(--panna-chiaro)]/20 outline-none focus:border-[var(--rosso)] transition-colors font-poppins text-base"
                        placeholder="es. 1972" />
                    </div>
                    <div className="col-span-2">
                      <label className="block text-[10px] uppercase tracking-[0.2em] text-[var(--panna-chiaro)]/40 mb-1 font-poppins font-medium">Note (opzionale)</label>
              ...
              {/* Prezzo e CTA */}
              <div className="flex flex-col md:flex-row items-stretch md:items-center gap-0 border-2 border-[var(--nero)]">
                <div className="flex-1 px-8 py-6 border-b-2 md:border-b-0 md:border-r-2 border-[var(--nero)]">
                  <p className="font-poppins text-[10px] uppercase tracking-[0.2em] text-[var(--nero)]/40 mb-1">Totale</p>
                  <p className="font-droid text-4xl text-[var(--nero)]">€20,00</p>
                  <p className="font-poppins text-xs text-[var(--nero)]/40 mt-1">3 notti · camping incluso · {formData.tipo === 'volkswagen' ? 'Zona A' : 'Zona B'}</p>
                </div>
                <button type="submit"
                  className="flex-1 px-8 py-6 bg-[var(--rosso)] text-white font-droid uppercase tracking-[0.15em] text-sm hover:bg-[var(--bordeaux)] transition-colors flex items-center justify-center gap-3">
                  <CreditCard size={18} />
            ...
            <div>
              ...
              <div className="border-2 border-[var(--nero)] mb-10">
                <div className="grid grid-cols-2 divide-x-2 divide-[var(--nero)] border-b-2 border-[var(--nero)]">
                  <div className="p-6">
                    <p className="font-poppins text-[10px] uppercase tracking-[0.2em] text-[var(--nero)]/40 mb-2">Nominativo</p>
                    <p className="font-poppins font-semibold">{formData.nome} {formData.cognome}</p>
                  </div>
                  <div className="p-6">
                    <p className="font-poppins text-[10px] uppercase tracking-[0.2em] text-[var(--nero)]/40 mb-2">Veicolo</p>
                ...
                <div className="grid grid-cols-2 divide-x-2 divide-[var(--nero)]">
                  <div className="p-6">
                    <p className="font-poppins text-[10px] uppercase tracking-[0.2em] text-[var(--nero)]/40 mb-2">Tipo</p>
                    <p className="font-poppins font-semibold">
                      {formData.tipo === 'volkswagen' ? 'Volkswagen — Zona A' : 'Auto/Camper — Zona B'}
                    </p>
                  </div>
                  <div className="p-6">
                    <p className="font-poppins text-[10px] uppercase tracking-[0.2em] text-[var(--nero)]/40 mb-2">Totale</p>
              ...
              {loading && (
                <p className="text-center text-[var(--nero)]/40 font-poppins text-xs uppercase tracking-widest py-8">
                  ...
                  <PayPalButtons
                    ...
                    createOrder={(data, actions) => actions.order.create({
                      ...
                      purchase_units: [{
                        ...
                        description: `Matese Volks Camp 2026 — ${formData.tipo === 'volkswagen' ? 'Volkswagen' : 'Auto/Camper'}`,
                      }],
                    })}
            ...
            <div>
              ...
              <div className="border-2 border-[var(--nero)] mb-8">
                <div className="bg-[var(--nero)] px-8 py-4">
                  <p className="font-poppins text-[10px] uppercase tracking-[0.3em] text-[var(--panna-chiaro)]/40">
                    Codice biglietto
                  </p>
                </div>
