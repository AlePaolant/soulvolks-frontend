...
export default function ConcorsoPage() {
    ...
    return (
        <main className="page">
                            ...
                            {[0, 1, 2, 3].map(i => (
                                <div key={i} className={`foto-slot ${foto[i] ? 'filled' : ''}`}>
                                    <label className="dropzone">
                                        ...
                                        />
                                        {foto[i] ? <span className="filename">📷 {foto[i]!.name}</span> : <span>+ Foto {i + 1}</span>}
                                    </label>
                                    <input
                ...
                {step === 'payment' && manualChoice && (
                    <div className="payment manual-confirmed">
                        ...
                        {manualChoice === 'contanti' ? (
                            ...
                            <p>La tua iscrizione è registrata. Scrivi a <strong>info@soulvolks.it</strong> per ricevere l&apos;IBAN ed effettua il bonifico di <strong>€10,00</strong> entro <strong>48 ore</strong> per confermare la partecipazione.</p>
                        )}
                        <p className="small">Se il pagamento non risulta entro 48 ore, l&apos;iscrizione verrà annullata automaticamente.</p>
                    </div>
                )}
            ...
            <style jsx>{`
        .page { background: #8a9683; min-height: 100vh; padding: 2.5rem 1.25rem 4rem; font-family: Georgia, 'Times New Roman', serif; color: #2b2b2b; }
        .hero { max-width: 680px; margin: 0 auto 1.5rem; text-align: left; }
        .eyebrow { font-family: Arial, sans-serif; letter-spacing: 0.25em; font-size: 0.72rem; color: #f5f3ec; text-transform: uppercase; display: inline-block; margin-bottom: 0.4rem; }
        h1 { color: #e2572b; font-size: 2.1rem; line-height: 1.15; margin: 0 0 0.4rem; text-transform: uppercase; }
        .dates { font-family: Arial, sans-serif; color: #f5f3ec; font-size: 0.95rem; letter-spacing: 0.03em; margin: 0; }
        .stepper { max-width: 680px; margin: 0 auto 1.25rem; display: flex; list-style: none; padding: 0; gap: 0.5rem; font-family: Arial, sans-serif; }
        .stepper li { flex: 1; display: flex; align-items: center; gap: 0.4rem; font-size: 0.78rem; color: #eae7dc; opacity: 0.6; }
        ...
        .stepper li.complete:not(:last-child)::after { background: #3f5b3f; }
        .stepper .dot { width: 22px; height: 22px; border-radius: 50%; background: #f5f3ec; color: #2b2b2b; display: flex; align-items: center; justify-content: center; font-size: 0.72rem; font-weight: bold; flex-shrink: 0; position: relative; z-index: 1; }        .stepper li.active .dot { background: #e2572b; color: white; }
        .stepper li.complete .dot { background: #3f5b3f; color: white; }
        .card { max-width: 680px; margin: 0 auto; background: #f5f3ec; border-radius: 6px; padding: 1.75rem 2rem 2rem; box-shadow: 0 10px 30px rgba(0,0,0,0.18); border-top: 5px solid #e2572b; }
        .regolamento { font-family: Arial, sans-serif; margin-bottom: 1rem; }
        .regolamento summary { cursor: pointer; font-weight: bold; color: #e2572b; text-transform: uppercase; font-size: 0.95rem; padding: 0.3rem 0; }
        ...
        .rank { background: #e2572b; color: white; font-weight: bold; border-radius: 4px; padding: 0.1rem 0.5rem; font-size: 0.75rem; flex-shrink: 0; }
        .small { font-size: 0.82rem; color: #666; }
        .consenso { display: flex; align-items: flex-start; gap: 0.6rem; font-family: Arial, sans-serif; font-size: 0.88rem; background: #eae7dc; border-radius: 5px; padding: 0.8rem 1rem; margin: 1.2rem 0; }
        ...
        input, textarea { font-family: Arial, sans-serif; padding: 0.6rem 0.7rem; border: 1px solid #cfc9ba; border-radius: 5px; font-size: 0.92rem; background: white; }
        input:focus, textarea:focus { outline: 2px solid #e2572b; outline-offset: 1px; }
        .turnstile { margin: 1.3rem 0 0.5rem; }
        .btn-primary { background: #e2572b; color: white; border: none; padding: 0.85rem 1.6rem; border-radius: 5px; font-family: Arial, sans-serif; font-weight: bold; text-transform: uppercase; letter-spacing: 0.04em; cursor: pointer; margin-top: 1.2rem; width: 100%; }
        .btn-primary:disabled { opacity: 0.6; cursor: not-allowed; }
        .btn-secondary { background: white; color: #2b2b2b; border: 1px solid #cfc9ba; padding: 0.7rem 1rem; border-radius: 5px; font-family: Arial, sans-serif; font-weight: bold; font-size: 0.85rem; cursor: pointer; flex: 1; }
        .btn-secondary:disabled { opacity: 0.6; cursor: not-allowed; }
        .error { font-family: Arial, sans-serif; background: #fde2e2; color: #a02020; padding: 0.7rem 0.9rem; border-radius: 5px; font-size: 0.88rem; margin-bottom: 1rem; }
        ...
        .dropzone { border: 2px dashed #cfc9ba; border-radius: 6px; padding: 1.4rem 0.6rem; text-align: center; font-family: Arial, sans-serif; font-size: 0.85rem; color: #888; cursor: pointer; background: white; }
        ...
        .divider { display: flex; align-items: center; text-align: center; margin: 1.3rem 0; font-family: Arial, sans-serif; color: #999; font-size: 0.8rem; }
        .divider::before, .divider::after { content: ''; flex: 1; border-bottom: 1px solid #ddd; }
        .divider span { padding: 0 0.8rem; }
        .manual-options { display: flex; gap: 0.7rem; }
        .manual-confirmed p { font-family: Arial, sans-serif; line-height: 1.5; }
        .done, .manual-confirmed { text-align: center; padding: 1.5rem 0; }
        .check { display: inline-flex; align-items: center; justify-content: center; width: 56px; height: 56px; border-radius: 50%; background: #3f5b3f; color: white; font-size: 1.8rem; margin-bottom: 1rem; }
        ...
        .site-footer p { margin: 0.2rem 0; font-size: 0.8rem; letter-spacing: 0.03em; }
        .site-footer .credits { opacity: 0.65; font-size: 0.72rem; }
